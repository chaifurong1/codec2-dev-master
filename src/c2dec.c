...
/*
  ...
  这个程序的主要作用是对Codec 2库进行控制，支持不同的模式选择、比特错误模拟等功能。程序结构清晰，通过命令行参数进行不同功能的配置。
...
int main(int argc, char *argv[])
{
	...
	// 使用 getopt_long 函数解析命令行参数，同时处理了一些特殊的命令行选项。
    char* opt_string = "h:";
    ...
    else if ( (fin = fopen(argv[2],"rb")) == NULL ) {  // 打开输入比特文件失败，输出错误信息，然后退出程序
    ...
    else if ( (fout = fopen(argv[3],"wb")) == NULL ) { // 打开输出音频文件失败，输出错误信息，然后退出程序
	fprintf(stderr, "Error opening output speech file: %s: %s.\n",
         argv[3], strerror(errno));
	exit(1);
    }

    // Attempt to detect a .c2 file with a header 尝试检测是否为Codec2文件
	//  如果输入文件扩展名为.c2，则尝试检测Codec2文件头信息，否则根据命令行参数选择Codec 2模式。
    ...
    if ((ext != NULL) && (strcmp(ext, ".c2") == 0)) {
    ...
    } else { // 如果不是Codec2文件，根据命令行参数选择Codec 2模式
        ...
        fprintf(stderr, "Error in mode: %s.  Must be 3200, 2400, 1600, 1400, 1300, 1200, 900, 700C, 450, or 450PWB\n", argv[1]);
        exit(1);
        }
    ...
    nstart_bit = 0;
    nend_bit = nbit-1;
	// 比特错误模拟：根据命令行参数设置的比特错误模式，进行相应的比特错误模拟。
    while(1) {
        int option_index = 0;
	...
	//// 根据不同的比特错误模式进行处理
	if ((error_mode == UNIFORM) || (error_mode == UNIFORM_RANGE)) {  //均匀和范围内随机比特错误模拟
            assert(softdec == 0);
	...
	if (error_mode == TWO_STATE) {
        ...
        //使用Codec 2库进行音频解码，然后将解码后的音频数据写入输出文件。
	...
	// 资源清理：在程序结束时销毁Codec 2对象并释放分配的内存，关闭输入、输出文件。
...
// 打印程序的使用说明，包括支持的命令行选项和参数。
void print_help(const struct option* long_options, int num_opts, char* argv[])
{
	int i;
	char *option_parameters;
	fprintf(stderr, "\nc2dec - Codec 2 decoder and bit error simulation program\n"
		"usage: %s 3200|2400|1600|1400|1300|1200|900|700C|450|450PWB InputFile OutputRawFile [OPTIONS]\n\n"
                "Options:\n", argv[0]);
