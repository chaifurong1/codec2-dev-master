...
void codec2_encode_900(struct CODEC2 *c2, unsigned char * bits, short speech[]);
void codec2_decode_900(struct CODEC2 *c2, short speech[], const unsigned char * bits);
...
struct CODEC2 * codec2_create(int mode)
{
    ...
    if (false == ( CODEC2_MODE_ACTIVE(CODEC2_MODE_3200, mode) 
		   ...
		   || CODEC2_MODE_ACTIVE(CODEC2_MODE_1200, mode)
                   || CODEC2_MODE_ACTIVE(CODEC2_MODE_900, mode)
    ...
    if ( CODEC2_MODE_ACTIVE(CODEC2_MODE_900, c2->mode)) {
    ...
    if ( CODEC2_MODE_ACTIVE(CODEC2_MODE_900, c2->mode))
    {
	c2->encode = codec2_encode_900;
	c2->decode = codec2_decode_900;
    }
    if ( CODEC2_MODE_ACTIVE(CODEC2_MODE_700C, c2->mode))
    {
...
/*---------------------------------------------------------------------------*\
  ...
  FUNCTION....: codec2_encode_900
  AUTHOR......: cfr
  DATE CREATED: 2024.2

  Version c of 900 bit/s codec that uses newamp1 fixed rate VQ of amplitudes.
  ...
  frame 1: nothing
  frame 2: nothing
  frame 3: 26 bit 2 stage VQ (13 bits/stage), 4 bits energy, 
  ...
  The bit allocation is:
    ...
    -----------------------------------------------------------
    Harmonic magnitudes (rate k VQ)     0         26        26
    Energy                              0          4         4
...
void codec2_encode_900(struct CODEC2 *c2, unsigned char * bits, short speech[])
{
    ...
    }//通过 analyse_one_frame 函数对输入的语音信号进行分析，得到 MODEL 结构体。

    int K = 20;
    float rate_K_vec[K], mean;
    float rate_K_vec_no_mean[K], rate_K_vec_no_mean_[K];

    //调用 newamp1_model_to_indexes 函数，将 MODEL 结构体转换为索引数组和相应的特征向量。newamp1.c文件里
...
#endif
     // 将特征向量和索引数组按照指定的格式打包到输出的 bit 流中。
    pack_natural_or_gray(bits, &nbit, indexes[0], 13, 0);//主要改的是这个前两个indexes[0]和indexes[1]
    pack_natural_or_gray(bits, &nbit, indexes[1], 13, 0);
...
/*---------------------------------------------------------------------------*\
  ...
  FUNCTION....: codec2_decode_900
  AUTHOR......: David Rowe
  DATE CREATED: August 2015

  Decodes frames of 36 bits into 320 samples (40ms) of speech.
...
void codec2_decode_900(struct CODEC2 *c2, short speech[], const unsigned char * bits)
{
    ...
    /* unpack bits from channel ------------------------------------*/
  //从输入的 bit 流中解包得到索引数组。
    indexes[0] = unpack_natural_or_gray(bits, &nbit, 13, 0);
    indexes[1] = unpack_natural_or_gray(bits, &nbit, 13, 0);
 ...
 //调用 newamp1_indexes_to_model 函数，将索引数组转换为 MODEL 结构体和相应的频谱信息。
   ...
   for(i=0; i<M; i++) {
       if (c2->fmlfeat != NULL) {
       /* We use standard nb_features=55 feature records for compatibility with train_lpcnet.py 为了与train_lpcnet.py兼容，我们使用标准的nb_features=55个特征记录*/
       float features[55] = {0};
       ...
       //调用 synthesise_one_frame 函数，根据 MODEL 结构体和频谱信息合成语音信号。
   }
}
...
float codec2_energy_900(struct CODEC2 *c2, const unsigned char * bits)
{
    int     indexes[4];
    unsigned int nbit = 0;
...
float codec2_get_energy(struct CODEC2 *c2, const unsigned char *bits)
{
    ...
    assert(
       ...
       ( CODEC2_MODE_ACTIVE(CODEC2_MODE_900, c2->mode)) ||
...
void synthesise_one_frame(struct CODEC2 *c2, short speech[], MODEL *model, COMP Aw[], float gain)
{
    ...
    if ( CODEC2_MODE_ACTIVE(CODEC2_MODE_900, c2->mode) || CODEC2_MODE_ACTIVE(CODEC2_MODE_700C, c2->mode) || CODEC2_MODE_ACTIVE(CODEC2_MODE_450, c2->mode) || CODEC2_MODE_ACTIVE(CODEC2_MODE_450PWB, c2->mode)  ) {
