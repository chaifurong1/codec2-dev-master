...
float rate_K_mbest_encode(int *indexes, float *x, float *xq, int ndim, int mbest_entries)
{
  ...
  /* Stage 1 */
  // 在第一阶段，使用第一个码本（codebook1）对输入向量（x）进行矢量量化
  mbest_search(codebook1, x, ndim, newamp1vq_cb[0].m, mbest_stage1, index);

  /* Stage 2 */
  // 在第二阶段，对第一阶段的结果使用第二个码本（codebook2）进行矢量量化
...
/*---------------------------------------------------------------------------* \
  ...
  在newamp1_model_to_indexes函数中，indexes 数组是用来存储矢量量化后的索引的。每个索引对应一个特定的特征向量（或者说模型的一部分）。下面是 indexes 数组中各元素的含义：

  indexes[0]: 第一阶段矢量量化的结果索引，通常对应于第一个码本（codebook1）中的一个码字。
  indexes[1]: 第二阶段矢量量化的结果索引，通常对应于第二个码本（codebook2）中的一个码字。
  indexes[2]: 标量量化均值的索引，用于表示均值在一个特定的码本（可能是 newamp1_energy_cb[0].cb）中的位置。
  indexes[3]: 标量量化周期参数 Wo 的索引。对于浊音帧，该索引来自 encode_log_Wo 函数，表示周期参数 Wo 的量化值。对于非浊音帧，索引为 0。
  特征向量则包括以下几部分：

  rate_K_vec: 速率 K 矢量，通过对变速率 L 模型参数进行转换获得。
  rate_K_vec_no_mean: 从速率 K 矢量中减去均值得到的去均值速率 K 矢量。
  rate_K_vec_no_mean_: 第二阶段矢量量化的结果，是去均值速率 K 矢量的矢量量化版本。
\*---------------------------------------------------------------------------*/
                              ...
                              )
{
    ...
    //使用resample_const_rate_f函数将变速率L的模型参数转换为固定速率K的参数。这一步是为了适应VQ（矢量量化）的要求

    /* remove mean *//* 计算速率K矢量的均值 计算速率K矢量的均值，并将均值从原始矢量中减去，得到去均值的速率K矢量。去均值可以帮助提高矢量量化的性能。*/
    float sum = 0.0;
    for(k=0; k<K; k++)
        sum += rate_K_vec[k];   
    *mean = sum/K;
    ...
    /* update and optionally run "front eq" equaliser on before VQ 更新并选择性地运行“前向均衡器”*/
    ...
    /* running sum of squared error for variance calculation 计算均方误差用于方差计算*/
    for(k=0; k<K; k++)
        *se += (float)pow(rate_K_vec_no_mean[k]-rate_K_vec_no_mean_[k],2.0);

    /* scalar quantise mean (effectively the frame energy) 对帧能量进行标量量化，并计算索引*/
    float w[1] = {1.0};
    float se_mean;
    ...
    /* scalar quantise Wo.  We steal the smallest Wo index to signal
       an unvoiced frame对周期参数Wo进行标量量化，索引用于表示非浊音帧 */
    if (model->voiced) {
